...
axs.scatter(x0Array[x0Array!=0],
   f1Array[f1Array!=0],
   label='probx',color='C0',alpha=0.5)
axs.legend()
axs.grid()
axs.set_xlim([-.1,np.pi+.1])
axs.set_ylim([-.05,.65])

fig.savefig('AcceptanceRejectionPlotf1.pdf')
...
axs.scatter(x0Array2[x0Array2!=0],
   f1Array2[f1Array2!=0],
   label='probx2',color='C0',alpha=0.5)
axs.legend()
axs.grid()
axs.set_xlim([-.1,np.pi+.1])
axs.set_ylim([-.05,.65])

fig.savefig('AcceptanceRejectionPlotf2.pdf')
...
with open('AcceptanceRejectionTable.tex',
          'w') as tf:
